...
import nvidia.dali.backend_impl
...
def log(boxes, labels, ids):
    for i, id in enumerate(ids):
        print(id, boxes[i], labels[i])
...
class COCOPipeline(Pipeline):
    def __init__(self, batch_size, file_root, annotations_file, default_boxes,
        ...
        # plugin_manager.load_library("/home/mahdi/CLionProjects/untitled2/cmake-build-debug/libcustomdummy.so")
        # self.op = dali.ops.CustomDummy(bbox_layout="xyXY")
...
to_torch_type = {
    dali.backend_impl.types._DALIDataType.FLOAT : torch.float32,
    dali.backend_impl.types._DALIDataType.FLOAT64 : torch.float64,
    dali.backend_impl.types._DALIDataType.FLOAT16 : torch.float16,
    dali.backend_impl.types._DALIDataType.UINT8   : torch.uint8,
    dali.backend_impl.types._DALIDataType.INT8    : torch.int8,
    dali.backend_impl.types._DALIDataType.INT16   : torch.int16,
    dali.backend_impl.types._DALIDataType.INT32   : torch.int32,
    dali.backend_impl.types._DALIDataType.INT64   : torch.int64
}
...
class DALICOCOIterator(object):
    ...
    def __next__(self):
        ...
        for i in range(self._num_gpus):
            ...
            images_torch_type = to_torch_type[images[0].dtype]
            bboxes_torch_type = to_torch_type[bboxes[0][0].dtype]
            labels_torch_type = to_torch_type[labels[0][0].dtype]
